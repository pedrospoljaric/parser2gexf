...
for n1 in lsNodes:
	for n2 in lsNodes:	
		if (n2 != n1):		
			...
			dicBLinks[str(n1) + '_' + str(n2)] = [[], [], []] # []start []end []weight
...
for lb1 in range(len(dlb)):
	...
	for bMsg in range(6, len(bline1), 3):
		...
		if len((dicBLinks[nodesLink])[0]) == 0:
		
			(dicBLinks[nodesLink])[0].append(float(bline1[bMsg+1]))
			(dicBLinks[nodesLink])[1].append(float(bline1[bMsg+2]))
			(dicBLinks[nodesLink])[2].append(float(1))
			print '+1'
			
		else:
			
			t = -1
			while float(bline1[bMsg+1]) < float(((dicBLinks[nodesLink])[0])[t]) and float(((dicBLinks[nodesLink])[0])[t]) != float(((dicBLinks[nodesLink])[0])[0]):
				t -= 1
			
			for i in range(t, 0):
			
				if float(((dicBLinks[nodesLink])[1])[i]) > float(bline1[bMsg+1]):
				
					newStart = float(bline1[bMsg+1])
					newEnd = min(float(((dicBLinks[nodesLink])[1])[i]), float(bline1[bMsg+2]))
					newWeight = float(((dicBLinks[nodesLink])[2])[i]) + 1
					bline1[bMsg+2] = max(float(bline1[bMsg+2]), float(((dicBLinks[nodesLink])[1])[i]))
					bline1[bMsg+1] = newEnd
					((dicBLinks[nodesLink])[1])[i] = newStart
					if (bline1[bMsg+2] < float(((dicBLinks[nodesLink])[1])[i])): bline1[2] = float(((dicBLinks[nodesLink])[2])[i])
					
					(dicBLinks[nodesLink])[0].append(newStart)
					(dicBLinks[nodesLink])[1].append(newEnd)
					(dicBLinks[nodesLink])[2].append(newWeight)
...
for item in dicLinks:
	...
	for i in range(0, len((dicLinks[item])[0])):
		...
		e.addAttribute(idLinkType, "P2P")
		print id

for itemB in dicBLinks:

	for i in range(0, len((dicBLinks[itemB])[0])):
		
		id += 1
		print itemB
		srcdst = itemB.split('_')
		edgeDuration = float(((dicBLinks[itemB])[1])[i]) - float(((dicBLinks[itemB])[0])[i])
		e = graph.addEdge(str(id), str(srcdst[0]), str(srcdst[1]), float(((dicBLinks[itemB])[2])[i]), str(((dicBLinks[itemB])[0])[i]), str(((dicBLinks[itemB])[1])[i]), str(itemB))
		e.addAttribute(idTotalEdgeDuration, str(edgeDuration))
		e.addAttribute(idLinkType, "Collective")
		print id
